...
"[$(Get-Date -Format o)] ALL DONE Stage8S1 pipeline" | Out-File -FilePath $masterLog -Encoding UTF8 -Append$ErrorActionPreference = 'Stop'
...
Run-Step -Name 'stage8s1_tiny_img512_wsampler_mask135_cls095_phaseA_40ep' -Command (
    "& '$py' '$trainDir\train_stage5a.py' " +
    "--backbone 'swin_tiny_patch4_window7_224' --img_size 512 --batch_size 1 --epochs 40 --lr 2.5e-5 --num_classes 104 " +
    "--loss_cls_weight 0.95 --loss_mask_weight 1.35 --mask_head_mode binary --boundary_loss_weight 0.0 " +
    "--max_train_batches 2000 --max_val_batches 500 --use_weighted_sampler " +
    "--hf_dataset_dir '$dataset' --output_dir '$stage8Out' --run_name 'stage8s1_tiny_img512_wsampler_mask135_cls095_phaseA_40ep' " +
    "--device cuda --checkpoint_every 5 --resume_from '$ckptStart' --class_weights_file '$weightsFile'"
)

$bestA = "$stage8Out\stage8s1_tiny_img512_wsampler_mask135_cls095_phaseA_40ep\best_stage8s1_tiny_img512_wsampler_mask135_cls095_phaseA_40ep.pth"
if (!(Test-Path $bestA)) {
    throw "Missing checkpoint for phase A eval: $bestA"
}

Run-Step -Name 'stage8s1_eval_tiny_img512_wsampler_mask135_cls095_phaseA_40ep_fullval' -Command (
    "& '$py' '$trainDir\eval_stage5b.py' " +
    "--hf_dataset_dir '$dataset' --output_dir '$stage8EvalOut' --run_name 'stage8s1_eval_tiny_img512_wsampler_mask135_cls095_phaseA_40ep_fullval' " +
    "--backbone 'swin_tiny_patch4_window7_224' --img_size 512 --batch_size 1 --num_classes 104 --device cuda --max_val_batches 0 " +
    "--checkpoint '$bestA'"
)

Run-Step -Name 'stage8s1_tiny_img512_wsampler_mask135_cls095_phaseB_30ep' -Command (
    "& '$py' '$trainDir\train_stage5a.py' " +
    "--backbone 'swin_tiny_patch4_window7_224' --img_size 512 --batch_size 1 --epochs 30 --lr 1.5e-5 --num_classes 104 " +
    "--loss_cls_weight 0.95 --loss_mask_weight 1.35 --mask_head_mode binary --boundary_loss_weight 0.0 " +
    "--max_train_batches 2000 --max_val_batches 500 --use_weighted_sampler " +
    "--hf_dataset_dir '$dataset' --output_dir '$stage8Out' --run_name 'stage8s1_tiny_img512_wsampler_mask135_cls095_phaseB_30ep' " +
    "--device cuda --checkpoint_every 5 --resume_from '$bestA' --class_weights_file '$weightsFile'"
)

$bestB = "$stage8Out\stage8s1_tiny_img512_wsampler_mask135_cls095_phaseB_30ep\best_stage8s1_tiny_img512_wsampler_mask135_cls095_phaseB_30ep.pth"
if (!(Test-Path $bestB)) {
    throw "Missing checkpoint for phase B eval: $bestB"
}

Run-Step -Name 'stage8s1_eval_tiny_img512_wsampler_mask135_cls095_phaseB_30ep_fullval' -Command (
    "& '$py' '$trainDir\eval_stage5b.py' " +
    "--hf_dataset_dir '$dataset' --output_dir '$stage8EvalOut' --run_name 'stage8s1_eval_tiny_img512_wsampler_mask135_cls095_phaseB_30ep_fullval' " +
    "--backbone 'swin_tiny_patch4_window7_224' --img_size 512 --batch_size 1 --num_classes 104 --device cuda --max_val_batches 0 " +
    "--checkpoint '$bestB'"
)

$evalA = "$stage8EvalOut\stage8s1_eval_tiny_img512_wsampler_mask135_cls095_phaseA_40ep_fullval\eval_summary.json"
$evalB = "$stage8EvalOut\stage8s1_eval_tiny_img512_wsampler_mask135_cls095_phaseB_30ep_fullval\eval_summary.json"
if (!(Test-Path $evalA) -or !(Test-Path $evalB)) {
    throw "Missing eval summary for A/B gate: $evalA or $evalB"
}
...
def choose(a_name, a_ckpt, a_metrics, b_name, b_ckpt, b_metrics):
    ma = float(a_metrics['val_mIoU'])
    mb = float(b_metrics['val_mIoU'])
    ca = int(a_metrics.get('nonzero_non_bg_classes', 0))
    cb = int(b_metrics.get('nonzero_non_bg_classes', 0))
    if mb > ma + eps:
        return {'winner': b_name, 'checkpoint': b_ckpt, 'val_mIoU': mb, 'nonzero_non_bg_classes': cb}
    if ma > mb + eps:
        return {'winner': a_name, 'checkpoint': a_ckpt, 'val_mIoU': ma, 'nonzero_non_bg_classes': ca}
    if cb >= ca:
        return {'winner': b_name, 'checkpoint': b_ckpt, 'val_mIoU': mb, 'nonzero_non_bg_classes': cb}
    return {'winner': a_name, 'checkpoint': a_ckpt, 'val_mIoU': ma, 'nonzero_non_bg_classes': ca}
...
a = read(r'''$evalA''')
b = read(r'''$evalB''')
print(json.dumps(choose('A', r'''$bestA''', a, 'B', r'''$bestB''', b)))
"@

$gateABObj = $gateAB | ConvertFrom-Json
$ckptGate = [string]$gateABObj.checkpoint
"[$(Get-Date -Format o)] GATE_AB winner=$($gateABObj.winner) mIoU=$($gateABObj.val_mIoU) coverage=$($gateABObj.nonzero_non_bg_classes)" | Out-File -FilePath $masterLog -Encoding UTF8 -Append

if (!(Test-Path $ckptGate)) {
    throw "Gate A/B checkpoint missing: $ckptGate"
}

Run-Step -Name 'stage8s1_tiny_img512_wsampler_mask135_cls095_phaseC_20ep' -Command (
    "& '$py' '$trainDir\train_stage5a.py' " +
    "--backbone 'swin_tiny_patch4_window7_224' --img_size 512 --batch_size 1 --epochs 20 --lr 1.0e-5 --num_classes 104 " +
    "--loss_cls_weight 0.95 --loss_mask_weight 1.35 --mask_head_mode binary --boundary_loss_weight 0.0 " +
    "--max_train_batches 2000 --max_val_batches 500 --use_weighted_sampler " +
    "--hf_dataset_dir '$dataset' --output_dir '$stage8Out' --run_name 'stage8s1_tiny_img512_wsampler_mask135_cls095_phaseC_20ep' " +
    "--device cuda --checkpoint_every 5 --resume_from '$ckptGate' --class_weights_file '$weightsFile'"
)

$bestC = "$stage8Out\stage8s1_tiny_img512_wsampler_mask135_cls095_phaseC_20ep\best_stage8s1_tiny_img512_wsampler_mask135_cls095_phaseC_20ep.pth"
if (!(Test-Path $bestC)) {
    throw "Missing checkpoint for phase C eval: $bestC"
}

Run-Step -Name 'stage8s1_eval_tiny_img512_wsampler_mask135_cls095_phaseC_20ep_fullval' -Command (
    "& '$py' '$trainDir\eval_stage5b.py' " +
    "--hf_dataset_dir '$dataset' --output_dir '$stage8EvalOut' --run_name 'stage8s1_eval_tiny_img512_wsampler_mask135_cls095_phaseC_20ep_fullval' " +
    "--backbone 'swin_tiny_patch4_window7_224' --img_size 512 --batch_size 1 --num_classes 104 --device cuda --max_val_batches 0 " +
    "--checkpoint '$bestC'"
)

$evalC = "$stage8EvalOut\stage8s1_eval_tiny_img512_wsampler_mask135_cls095_phaseC_20ep_fullval\eval_summary.json"
if (!(Test-Path $evalC)) {
    throw "Missing eval summary for final gate: $evalC"
}
...
gate_ab = choose('A', r'''$bestA''', a, 'B', r'''$bestB''', b)
gate_final = choose(gate_ab['winner'], gate_ab['checkpoint'], gate_ab, 'C', r'''$bestC''', c)

print(json.dumps({
    'winner': gate_final['winner'],
    'checkpoint': gate_final['checkpoint'],
    'winner_mIoU': gate_final['val_mIoU'],
    'winner_nonzero_non_bg_classes': gate_final['nonzero_non_bg_classes'],
    'phaseA_mIoU': float(a['val_mIoU']),
    'phaseB_mIoU': float(b['val_mIoU']),
    'phaseC_mIoU': float(c['val_mIoU']),
    'phaseA_nonzero_non_bg_classes': int(a.get('nonzero_non_bg_classes', 0)),
    'phaseB_nonzero_non_bg_classes': int(b.get('nonzero_non_bg_classes', 0)),
    'phaseC_nonzero_non_bg_classes': int(c.get('nonzero_non_bg_classes', 0)),
    'gate_ab_winner': gate_ab['winner'],
    'gate_ab_checkpoint': gate_ab['checkpoint'],
    'epsilon': eps,
}))
"@
...
$gateObj | ConvertTo-Json -Depth 6 | Out-File -FilePath $gateDecision -Encoding UTF8
"[$(Get-Date -Format o)] GATE_FINAL winner=$($gateObj.winner) mIoU=$($gateObj.winner_mIoU) coverage=$($gateObj.winner_nonzero_non_bg_classes)" | Out-File -FilePath $masterLog -Encoding UTF8 -Append
