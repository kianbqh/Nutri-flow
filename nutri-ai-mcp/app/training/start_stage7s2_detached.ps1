...
$existing = Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -match 'run_stage7s2\.ps1' }
if ($existing) {
    Write-Output "Stage7S2 orchestrator already running: $($existing.ProcessId -join ', ')"
}
else {
    $s7StdOut = "$serviceLogDir\stage7s2_orchestrator_stdout.log"
    $s7StdErr = "$serviceLogDir\stage7s2_orchestrator_stderr.log"
    $s7Script = "$root\app\training\run_stage7s2.ps1"
    ...
    Write-Output "Started Stage7S2 orchestrator PID: $($proc.Id)"
}
